...
### 1. 综合测试 (comprehensive.test.ts) - 48 个测试
...
### 2. 应用级别事件钩子 (app-events.test.ts) - 20 个测试
...
#### 2.13 triggerHealthCheck 钩子

- ✅ 应该调用 onHealthCheck 钩子
- ✅ 应该合并多个插件的健康检查结果
- ✅ 应该正确计算整体状态

#### 2.14 triggerHotReload 钩子
...
### 6. 错误隔离 (error-isolation.test.ts) - 6 个测试
...
### 12. 事件发射器 (event-emitter.test.ts) - 10 个测试
...
`@dreamer/plugin`
插件管理库的测试覆盖全面，所有核心功能和高级功能都有对应的测试用例。
